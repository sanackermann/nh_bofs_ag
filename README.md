...
| **sc_config**                     | Configures a local or remote **existing Windows service** (BinPath, ErrorMode, StartMode).                                                              | `sc_config <SVCNAME> <BINPATH> <ERRORMODE> <STARTMODE> [HOSTNAME]`                                                                             |
| **sc_create**                     | Creates a **new Windows service** on the target host (including display name and description); optional: remote and service type.                       | `sc_create <SVCNAME> <DISPLAYNAME> <BINPATH> <DESCRIPTION> <ERRORMODE> <STARTMODE> [TYPE] [HOSTNAME]`                                          |
| **sc_delete**                     | Deletes a Windows service locally or on a remote host.                                                                                                  | `sc_delete <SVCNAME> [HOSTNAME]`                                                                                                               |
| **sc_stop**                       | Stops a Windows service locally or on a remote host.                                                                                                    | `sc_stop <SVCNAME> [HOSTNAME]`                                                                                                                 |
| **sc_start**                      | Starts a Windows service locally or on a remote host.                                                                                                   | `sc_start <SVCNAME> [HOSTNAME]`                                                                                                                |
| **enableuser**                    | Activates/unblocks a user account (domain account or local account, depending on the DOMAIN parameter).                                                 | `enableuser <USERNAME> [DOMAIN]`                                                                                                               |
| **persist_service**               | Creates **persistence via a Windows service**: optionally uploads a service binary, creates the `agcssvc` service, sets failure recovery and starts it. | `persist_service <SERVICE_BINARY_ON_ATTACKER_COMPUTER>`                                                                                        |
| **spray_ad**                      | Performs a Kerberos or LDAP password spraying attack against Active Directory.                                                                          | `spray_ad <PASSWORD> <OPT:USERNAME_FILTER> <OPT:--ldap>`                                                                                       |
...
| Command             | Description                                                                     | Usage                                                                                  |
| ------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| **sql-whoami**      | Gather information about logged in user, mapped user and roles on a SQL server. | `sql-whoami server [database] [linkedserver] [impersonate]`                            |
| **sql-info**        | Gather general information about a SQL server.                                  | `sql-info server [database]`                                                           |
| **sql-enablexp**    | Enable xp_cmdshell.                                                             | `sql-enablexp server [database] [linkedserver] [impersonate]`                          |
| **sql-xpcmd**       | Execute a system command via xp_cmdshell.                                       | `sql-xpcmd server command [database] [linkedserver] [impersonate]`                     |
